...
func New(cfg config.Config, client *http.Client, logger *logging.Logger) (provider.Provider, error) {
	...
	case config.ProtocolAnthropic:
		return anthropic.New(anthropic.Options{BaseURL: u, APIKey: cfg.APIKey, Model: cfg.Model, HTTPClient: client, Logger: logger}), nil
	case config.ProtocolOpenAI:
		return openai.New(openai.Options{BaseURL: u, APIKey: cfg.APIKey, Model: cfg.Model, HTTPClient: client, Logger: logger}), nil
	default:
