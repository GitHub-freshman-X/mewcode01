...
func (t *RemoteToolAdapter) Execute(ctx context.Context, input json.RawMessage) tools.Result {
	...
	if _, err := tools.Validate(meta.Schema, input); err != nil {
		t.logger.Error("MCP tool call failed", logging.Fields{"stage": "call", "status": "validation_failed", "remote_tool": t.remote.Name, "tool": meta.Name})
		return tools.Failure(meta.Name, err.Type, err.Message, err.Details)
	}
	t.logger.Info("MCP tool call started", logging.Fields{"stage": "call", "status": "started", "remote_tool": t.remote.Name, "tool": meta.Name})
	result, err := t.client.CallTool(ctx, t.remote.Name, input)
	if err != nil {
		t.logger.Error("MCP tool call failed", logging.Fields{"stage": "call", "status": "rpc_failed", "remote_tool": t.remote.Name, "tool": meta.Name})
		return tools.Failure(meta.Name, tools.ErrorExecution, "MCP tool call failed", map[string]any{"cause": safeError(err)})
	}
	if result.IsError {
		t.logger.Error("MCP tool call failed", logging.Fields{"stage": "call", "status": "tool_error", "remote_tool": t.remote.Name, "tool": meta.Name})
	...
	t.logger.Info("MCP tool call succeeded", logging.Fields{"stage": "call", "status": "succeeded", "remote_tool": t.remote.Name, "tool": meta.Name})
