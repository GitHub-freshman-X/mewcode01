...
| tool_search | `auto`（默认）、`enabled` 或 `disabled`；原生 Tool Search 优先，本地 ToolSearch 兜底。 |
...
`tool_search: auto` 与 `enabled` 会在官方支持的 Anthropic 或 OpenAI Responses 模型上启用服务端 Tool Search：完整 MCP schema 仍提交给 Provider，并由 `defer_loading` 控制模型可见性。其他模型和兼容网关使用本地 ToolSearch：初始请求不发送 MCP schema，模型先调用 `tool_search` 取得定义，再通过 `mcp_call` 由本地 MCP Client 执行。`disabled` 始终发送完整工具定义。
