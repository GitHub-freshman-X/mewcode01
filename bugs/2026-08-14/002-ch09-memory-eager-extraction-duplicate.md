...
2026-08-14 10:18 的手工测试日志仍显示旧提取提示（未携带既有索引），说明 fixture 正在运行修复前构建；需要重新构建并启动测试二进制后才能验证本修复。

重新构建后的 10:26 手工日志表明修复已生效，但仍进行了一次治理。根因是节流标记位于 `memory/.consolidate-last-success`：删除整个 `memory/`（包括 `MEMORY.md`）会同时删除标记。下一次提取重新创建该目录后，sessions 数量已达到阈值，因而立即治理。后续应将治理状态移到不随记忆内容一同删除的位置。
