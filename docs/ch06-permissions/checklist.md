...
- [ ] **AC5 规则优先级正确**：同一调用被多层规则覆盖时，会话级高于项目级，项目级高于用户级，显式规则高于权限模式。（验证：运行 `go test ./internal/permissions -run 'RuleStore|Priority|ExplicitRule'`，期望每层移除后决策按优先级回落）
...
- [ ] **AC8 永久允许写入项目规则**：用户选择永久允许后，`<项目根>/.mewcode/permissions.yaml` 新增 allow 规则；重新加载后相同调用命中项目级规则，用户级文件不变。（验证：运行 `go test ./internal/permissions -run 'AppendProjectAllow|LoadRuleSet'`，期望项目文件内容和重载决策正确）
...
- [ ] **项目规则路径明确**：项目级规则固定为 `<项目根>/.mewcode/permissions.yaml`，不再读取 `permissions.local.yaml`。（验证：运行 `go test ./internal/permissions -run 'DefaultFilePaths'`，期望路径断言通过）
...
- [ ] **E2E 4：永久允许跨重载生效**：默认模式下模型请求一个未命中的副作用调用，用户选择永久允许；重建权限引擎后相同调用由项目级规则直接允许。（验证：运行权限配置集成测试，期望项目规则文件存在且重载后决策来源为 project）
