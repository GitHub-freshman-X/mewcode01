...
| 修改 | `internal/agent/{event,history,runner}.go` | 模式、事件、Plan 替换和循环编排 |
...
3. 写失败测试：单项超阈值写入会话专属目录、预览含大小和路径；聚合超限按长度降序替换且输出顺序不变；标记为持久化回读的结果不再次替换。
4. 实现 `ResultStore` 和 `PrepareResults`，限制预览长度并返回 `Persistence` 元数据。
5. 运行 Context 包测试。
