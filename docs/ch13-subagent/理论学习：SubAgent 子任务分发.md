...
![上下文污染终端噪声](https://feishu.cn/file/Xx42bybT8oFunBxj2gicrcsAnjf)
...
![一个上下文 vs 独立上下文](https://feishu.cn/file/BehpbPo41oIEh8xKljlcLCEknlf)
...
![Tool 接口与 Agent 同构](https://feishu.cn/file/KbIebItMLoJvpXxBeC9cVLmvnUe)
...
![Agent 工具委派子 Agent](https://feishu.cn/file/V8PpbSzOroS5oJx4R3ZcGnd5nUh)
...
![定义式与 Fork 式对比](https://feishu.cn/file/TWtHbGFi9oms5yxLxtYc15uanSg)
...
![只读专家能力边界](https://feishu.cn/file/P0y0bQfRfo1ZmOxn8OwcbBojnOf)
...
![定义式与 Fork 的对话历史差异](https://feishu.cn/file/XWlxbjGSQobjAEx8msscbBfankJ)
...
![Fork 命中 prompt cache](https://feishu.cn/file/I6E3b3hMao7gulxMRofczwaHnYe)
...
注意，有一个限制：Fork 子 Agent 不能再 Fork。它的工具集保留了 Agent 工具（继承自父 Agent），所以工具还在手上，拦截发生在调用那一刻，检测到 caller 是 Fork 路径产生的子 Agent 就直接报错，并提示改用 `subagent_type` 去 spawn 定义式子 Agent。主检测是 QuerySource 标记，它跟着工具实例走，不在对话历史里，所以对话压缩影响不到它，对话里那段 `FORK_BOILERPLATE_TAG` 标记扫描是第二道兜底，代价是它本身会随对话压缩被摘要掉。这是为了防止 Fork 链条无限延伸导致上下文爆炸。
...
![SubAgent 创建模式决策树](https://feishu.cn/file/DWBdbDek4ohpebxVRWhcmh9vncg)
...
![父子 Agent 的隔离与共享边界](https://feishu.cn/file/Q82ebrugzofUhExZzp8cdn06nPc)
...
![图片展示了Skill和Agent的定义对比。左侧为Skill定义，包含YAML结构定义（name、description、allowedTools）和Markdown说明文档，白名单优先；右侧为Agent定义，同样有YAML结构定义（name、description、tools、disallowedTools、model），但系统提示为Markdown，黑名单+白名单，且Agent定义字段比Skill多model。图片与上下文关系紧密，直观呈现了Skill和Agent在结构、字段及优先级上的差异。](https://feishu.cn/file/JEicbF6cGoc7xWxe1ifcW5vpnrh)
...
2. 用户级：{os.UserConfigDir()}/mewcode/agents/（跨平台用户配置目录）
3. 内置级：程序编译嵌入
...
![Agent 定义来源优先级](https://feishu.cn/file/LTYMbifwWoMUqQxusWpczFXZnpe)
...
![交互式主 Agent 与 RunToCompletion](https://feishu.cn/file/NdJtb66W3o4nhgxoZ0ScphJSnfc)
...
![能力边界与 dontAsk](https://feishu.cn/file/NeACb3TYpocVswxpBx2c61F9nYb)
...
具体来说，系统在两个层面做了硬限制。第一，Fork 不能再 Fork。子 Agent 身上带着 Fork 来源标记，再次 Fork 会直接报错。这防止了 Fork 链条无限延伸导致上下文爆炸。第二，后台 Agent 不能再 spawn Agent。系统有一个硬编码的后台工具白名单 `ASYNC_AGENT_ALLOWED_TOOLS`，后面「工具过滤的多层防线」一节会展开。这个白名单里不包含 Agent 工具本身，从根源上切断了后台嵌套的可能性。

![嵌套 SubAgent 阻断机制](https://feishu.cn/file/XI17b1SuCojQUtxV8j0cL1Q0nSg)

注意，这两条限制都是在能力层面做约束，不是靠一个硬编码的深度数字来卡。具体怎么实现，定义式和 Fork 走的是不同路径。定义式子 Agent 在工具列表层就被 `ALL_AGENT_DISALLOWED_TOOLS` 过滤掉了 Agent 工具，工具集里根本看不到它，自然调不了。Fork 路径不走这个过滤，Fork 子 Agent 的工具集保留了 Agent 工具（`cloneRegistryForFork` 复制全部工具），拦截放在调用那一刻，主检测是跟着工具实例走的 QuerySource 标记，它不在对话历史里，因此不受对话压缩影响，对话里的 Fork boilerplate 标记扫描作为第二道兜底。再叠加后台 Agent 白名单同样不含 Agent 工具，三条路径合起来，Fork 链条只能有一层，Fork 出来的那层还能再派定义式子 Agent，而定义式子 Agent 拿不到 Agent 工具，是这条链的终点。
...
![后台 Agent 的三种进入路径](https://feishu.cn/file/AOSHb32zvovhYKxPa2Oc5zvanFe)
...
![adoptRunning 前后台移交时间线](https://feishu.cn/file/IeqTbiOk4oK0AyxmsymcHMBQnYe)
...
![SubAgent 工具过滤层](https://feishu.cn/file/KdKHbo4e3oFA9fx17LgcrKS6ngb)
...
![内置 Agent 角色卡片](https://feishu.cn/file/GH3HbvpW5oRyBExoZbmc1DibnOf)
...
![Verification Agent 开关](https://feishu.cn/file/FGIObCxakoOCrGxPk1hc4Cu8n1a)
